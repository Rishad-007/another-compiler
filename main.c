...
void evaluate_three_address_code() {
    ThreeAddressCode* current = code_head;
    int executing = 1;  // Track if we should execute the current instruction
    char* next_label = NULL;  // For tracking where to jump
    ...
    while (current != NULL) {
        if (strcmp(current->op, "label") == 0) {
            if (next_label && strcmp(current->result, next_label) == 0) {
                executing = 1;
                next_label = NULL;
            }
        }
        else if (strcmp(current->op, "goto") == 0) {
            next_label = current->result;
            executing = 0;
        }
        else if (strcmp(current->op, "ifFalse") == 0) {
            int condition = get_symbol_value(current->arg1);
            if (!condition) {  // If condition is false, jump to the label
                next_label = current->arg2;
                executing = 0;
            }
        }
        else if (executing) {
            if (strcmp(current->op, "=") == 0) {
                ...
                int val2 = (current->arg2[0] >= '0' && current->arg2[0] <= '9') ? 
                          atoi(current->arg2) : get_symbol_value(current->arg2);
                int result;
                if (strcmp(current->op, "<") == 0) result = val1 < val2;
                else if (strcmp(current->op, ">") == 0) result = val1 > val2;
                else result = val1 == val2;
                set_symbol_value(current->result, result);
            } else if (strcmp(current->op, "printf") == 0) {
